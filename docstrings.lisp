...
(defun document-package (package &key output-file (standalone t) title (includes t))
...
(defun name-to-path (name)
  (cl-ppcre:regex-replace-all " " (string-downcase (alphanumize name)) "-"))

(defun document-system
    (chapter-name packages
     &key (dir *default-pathname-defaults*)
       (index-file (concatenate 'string
                                (name-to-path chapter-name)  "-sb-texinfo.texi"))
       (menu-offset 36)
       next previous top)
  "Calls `document-package' on PACKAGES creating a single index for CHAPTER-NAME.
Suitable for documenting a multi-package system as defined with
package-inferred-system."
  (labels ((doc-shortname (package)
             (let ((package (find-package package)))
               (or (car (package-nicknames package))
                   (package-name package))))
           (doc-name (package)
             (package-name (find-package package)))
           (doc-path-basename (package)
             (name-to-path (doc-shortname package))))
    (uiop/filesystem:with-current-directory (dir)
      (generate-includes "include/" packages)
      (let ((pkgs (mapcar
                   (lambda (package) ; Generate package docs.
                     (let ((title (doc-name package))
                           (base (doc-path-basename package)))
                       (document-package package
                                         :output-file (concatenate
                                                       'string
                                                       base
                                                       "-sb-texinfo.texi")
                                         :standalone nil
                                         :includes nil
                                         :title title)
                       (list package title base)))
                   packages)))
        (with-texinfo-file index-file
          (unless *inhibit-sb-texinfo-macros*
            (write-line "@include include/sb-texinfo.texinfo" *texinfo-output*)
            (terpri *texinfo-output*))
          (format *texinfo-output*
                  "@node ~@[~A~], ~@[~A~], ~@[~A~], ~A~%@appendix ~A~%~%"
                  chapter-name next previous top
                  chapter-name)
          (write-line "@menu" *texinfo-output*)
          (dolist (pkg pkgs)
          ...
          ;; Write Sections
          (loop :with last
             :for current := (pop pkgs)
             :for next := (car pkgs)
             :do (destructuring-bind (l l-title l-base)
                     (or last (list nil chapter-name nil))
                   (declare (ignorable l l-base))
                   (destructuring-bind (c c-title c-base) current
                     ...
                     (destructuring-bind (n n-title n-base)
                       ...
                       (format *texinfo-output*
                               "~%@node ~@[~A~], ~@[~A~], ~@[~A~], ~A
@section ~A
@include ~A-sb-texinfo.texi~%"
                               c-title n-title l-title chapter-name
                               c-title
                               c-base))))
               (setq last current)
             :until (null pkgs))))))
  (merge-pathnames index-file dir))
